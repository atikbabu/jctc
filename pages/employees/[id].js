...
export default function EditEmployeePage() {
  ...
  const fetchEmployee = async () => {
    try {
      ...
      setMessage(`❌ Error fetching employee: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (message && message.startsWith('❌ Error fetching employee')) {
    ...
    <div className="space-y-6 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
        ...
        <div className="flex space-x-4">
          ...
          <Link href="/employees" className="bg-gray-300 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-400">
            Cancel
          </Link>
        </div>
      </form>
