...
export default function CategoryListPage() {
    ...
    if (confirm('Are you sure you want to delete this category?')) {
      try {
        ...
        if (res.ok) {
          ...
          setMessage(`❌ Error deleting category: ${err.error || 'Something went wrong'}`);
        }
      } catch (error) {
  ...
  const handleExportPDF = async () => {
    ...
    doc.setFontSize(14);
    doc.text('Category List', pageWidth / 2, 60, { align: 'center' });
    ...
    doc.save(`category-list-${Date.now()}.pdf`);
  };
  ...
  return (
    <div className="p-6 space-y-4">
          ...
          <tbody>
            {categories.map((category) => (
              <tr key={category._id}>
                ...
                <td className="border border-gray-300 p-2">{category.description || 'N/A'}</td>
                <td className="border border-gray-300 p-2 relative text-center no-print">
                  ...
                  {openMenuId === category._id && (
                    <div className="absolute right-0 mt-2 w-32 bg-white border border-gray-200 rounded-md shadow-lg z-10">
                      <Link href={`/production/categories/${category._id}`} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                        Edit
                      </Link>
                      <button
            ...
            {categories.length === 0 && (
              <tr>
                <td colSpan="3" className="text-center text-gray-500 p-4">No categories found.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="no-print flex justify-end mt-4">
        <Link href="/production/categories/add" className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
          + Add Category
        </Link>
      </div>
    </div>
